...
def helmUpgrade() {
  sh """
    ...
    sudo helm upgrade --install ioc-ui ${env.chartRepoName}/${env.chartName} --namespace ${params.namespace} --create-namespace --version ${env.chartVersion} --kubeconfig ${env.kubeConfigDir} --debug --atomic --timeout 2m0s
  """
}
...
pipeline {
  ...
  environment {
    ...
    gitBranch = "main"
    gitRepoUrl = "https://github.com/pongsathorn-ph/ioc-ui.git"

    chartRepoName = "demo-repo"
    chartRepoUrl = "https://pongsathorn-ph.github.io/ioc-ui/helm-chart/"
    chartName = "ioc-ui-chart"
